...
int random ();
int play(char [][col], vector<int>&, int, int, int);
int checkboard(int*,vector<int>&, int, int, int, int);
...
void setboard(vector<int>&, const int, const int);
...
int main()
{
	...
	vector<int> boardspace(numbs);
	fill(boardspace.begin(), boardspace.end(), 0);
...
int play(char players[][col], vector<int>& boardspace, int biggestobstacles, int nump, int numbs) //this functions plays the game.
{
	int *playerpos;
	int winner = -1, store = 0, whosturn = 0, length1 = 0, length2= 0, row = 0, row2 = 1;
	char answer, temp;
	int x = 0; 
...
/*	for(int i = 0; i < nump; i++)// determines whos going first. 
		for (int j = 0; j < nump; j++)
			if ( sizeof(players[i]) < sizeof(players[j]) )
			{
				temp = *players[i];
				*players[i] = *players[j];
				*players[j] = temp;
	...
	i cant get swap players name to work, everything is fine is just that the players are not in order. shortest to longest
*/			
	cout <<"The current players are:" <<endl;// tells whos turn it is
	
	for(int pos = 0; pos < nump; pos++)
	{
		cout << "Player " << pos+1 << " is: ";
		for(int i = 0; i < col; i++)
		{
			if(players[pos][i] != '\0')
				cout << players[pos][i];
...
int checkboard(int* playerpos, vector<int>& boardspace , int whosturn, int biggestobsticle, int nump, int numbs)
...
void showpos(const char players[][col], const int *playerpos, int nump)//shows all the players position, if user says yes.
{
	...
	for(int pos = 0; pos < nump; pos++)
	{
		...
		cout <<" is in postion " << *(playerpos +pos) <<endl;
		cout << endl;
	}
...
void setboard(vector<int>& boardspace, const int obstacle,const int numbs)//fucntion puts the obstacles and go again in place.
{
