...
{
	int *boardspace;
	int nump, numbs, numobstacles, biggestobstacles, winner;
	char* players[100];
	...
	for(int playercount = 0; playercount < nump ; playercount++)
	{
		...
		cin.getline(players[playercount],10, ','); 
	}
...
int play(char players, int *boardspace, int biggestobstacles, int nump, int numbs) //this functions plays the game.
{
	int *playerpos;
...
void showpos(const char* players, const int *playerpos, int nump)//shows all the players position, if user says yes.
{
	for(int i = 0; i < nump; i++)
