...
int taketurns(int playerpos[], int& n, int whosturn)
{
	...
	playerpos[whosturn] = obstacle(playerpos, whosturn);
...
int random(int cards[])//picks cards at random and return it to the play function
{
	...
	if(cards[pick] == 5)
		cout << "You Pick the card and it says Lose A Turn!" <<endl<<endl;
		else if (cards[pick] == 6)
			cout << "You Pick the card and it Switch Postions with the other player!" <<endl<<endl;
				else
					cout << "You are moving " << cards[pick] <<" spaces" <<endl<<endl;
					
	return pick;
}

int obstacle(int playerpos[], int whosturn)
{
	int obstacle[20] = {0,0,2,0,0,1,0,3,0,0,0,0,0,2,0,0,0,2,0,1};
	int position = playerpos[whosturn] - 1;
	
	if(obstacle[position] != 0)
	{
		cout<<"You Hit A CONE!. You are moving back " << obstacle[position] << " spaces!" <<endl;
		playerpos[whosturn] = playerpos[whosturn] - obstacle[position];
	}
	
	playerpos[whosturn] = samepos(playerpos, whosturn);
...
int samepos(int playerpos[], int whosturn)
{
	int count = 0;
	
	while(count < 6 && playerpos[whosturn] != 0)
	{
		if(whosturn == count)
			count ++; 
		else if (playerpos[whosturn] == playerpos[count])
		{
			cout <<"You are in the same poistion as player " << count <<". Back to home you go! " <<endl;
